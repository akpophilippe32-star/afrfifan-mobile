...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _loadMessages() async {
    ...
    final messages = await _messagingService.fetchConversation(
  ...
  // ─────────────── ENVOI / RÉPONSE / ÉDITION / SUPPRESSION ───────────────
  ...
  Future<void> _sendContent(String content) async {
    ...
    if (reply != null) {
      ...
      success = await _messagingService.sendMessage(
  ...
  void _confirmDelete(Map<String, dynamic> message) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
          'Ce message sera supprimé pour vous et votre destinataire.',
              ...
              _menuTile(Icons.reply, 'Répondre', Colors.white, () {
                ...
                setState(() {
                  _replyTo = {
                    'id': message['id'],
                    'content': content,
                    'name': isMine ? 'Vous' : widget.otherUserName,
                  };
                  _editingMessage = null;
                });
  ...
  /// ✅ Clic sur le header → profil de la personne
  void _openUserProfile() {
    Navigator.push(
      context,
      MaterialPageRoute(
        // ⚠️ SI erreur "No named parameter 'creatorId'" :
        // ouvre creator_profile_screen.dart (lignes du haut) et remplace
        // 'creatorId' par le nom EXACT du paramètre déclaré dans cette classe.
        builder: (_) => CreatorProfileScreen(creatorId: widget.otherUserId),
      ),
    );
  }
  ...
  void _showEncryptionInfo() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
          'Personne en dehors de cet échange, pas même nous, ne peut lire ou écouter ce qui est envoyé.',
          style: TextStyle(color: Colors.grey.shade400, fontSize: 14),
        ),
        actions: [
          TextButton(
  ...
  void _onAttachmentTap(String label) {
    // TODO : brancher image_picker, file_picker, enregistreur audio, geolocator
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => _onAttachmentTap('Appels audio'),
          ),
          IconButton(
            icon: Icon(Icons.videocam_outlined, color: AppColors.primary),
            onPressed: () => _onAttachmentTap('Appels vidéo'),
          ),
        ],
      ),
      body: Column(
                    ...
                    itemBuilder: (context, index) {
                      if (index == 0) {
                        final firstDate = _messages.isNotEmpty
                            ? _messages[0]['created_at'].toString()
                      ...
                      if (i > 0) {
                        ...
                        if (!_isSameDay(previous, current)) {
                          separator =
                              _dateSeparator(message['created_at'].toString());
                        }
                      }
                            ...
                            decoration: InputDecoration(
                              ...
                              suffixIcon: IconButton(
                                ...
                                onPressed: () => setState(() {
                                  ...
                                  if (_showEmojis) _focusNode.unfocus();
                                }),
                              ),
                            ),
  ...
  Widget _dateSeparator(String createdAt) {
    return Center(
      child: Container(
        ...
        child: Text(
          _formatDateSeparator(createdAt).toUpperCase(),
          style: TextStyle(
            ...
            Flexible(
              child: RichText(
                ...
                text: TextSpan(
                  children: [
                    TextSpan(
                      text: 'Les messages et les appels sont chiffrés de bout en bout. ',
                      style: TextStyle(color: Colors.grey.shade400, fontSize: 13),
                    ),
                    TextSpan(
                      text: 'En savoir plus',
                      style: TextStyle(color: AppColors.primary, fontSize: 13),
                      recognizer: TapGestureRecognizer()..onTap = _showEncryptionInfo,
                    ),
                  ],
                ),
              ),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Text(
                '${_formatTime(createdAt)}${isEdited ? ' · modifié' : ''}',
              ...
              if (isMine) ...[
                ...
                Icon(
                  ...
                  size: 14,
                  color: isRead ? AppColors.primary : Colors.grey.shade600,
                ),
              ],
            ],
          ),
